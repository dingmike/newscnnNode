...
module.exports = class extends Base {
     ...
     */
    async getCart() {
        // 购物车下单buy_one=0; 立即购买buy_one=1;
        const cartList = await this.model('cart').where({user_id: think.userId, session_id: 1, buy_one: 0}).select();
        // 获取购物车统计信息
        let goodsCount = 0;
    ...
    async getBuyOne() {
        // 购物车下单buy_one=0; 立即购买buy_one=1;
        const cartList = await this.model('cart').where({user_id: think.userId, session_id: 1, buy_one: 1}).select();
        // 获取购物车统计信息
        let goodsCount = 0;
        let goodsAmount = 0.00;
        let checkedGoodsCount = 0;
    ...
    async addOneAction() {
        ...
        // 判断购物车中是否存在此规格商品
        const cartInfo = await this.model('cart').where({goods_id: goodsId, product_id: productId, buy_one: 1}).find();
        if (think.isEmpty(cartInfo)) {
            // 添加操作
    ...
    async updateAction() {
        ...
        const cartInfo = await this.model('cart').where({id: id, buy_one: 0}).find();
        // 只是更新number
        if (cartInfo.product_id === productId) {
            await this.model('cart').where({id: id, buy_one: 0}).update({
                number: number
            });
        ...
        if (think.isEmpty(newCartInfo)) {
            ...
            await this.model('cart').where({id: id, buy_one: 0}).update(cartData);
        } else {
            ...
            await this.model('cart').where({id: newCartInfo.id, buy_one: 0}).delete();
    ...
    async deleteAction() {
        ...
        await this.model('cart').where({product_id: {'in': productId}, buy_one: 0}).delete();

        return this.success(await this.getCart());
    }

    // 删除选中的购物车商品，批量删除 立即购买的商品
    async deleteBuyOneAction() {
        // let productId = this.post('productIds');

        await this.model('cart').where({buy_one: 1}).delete();
    ...
    async checkoutAction() {
        ...
        if (addressId) {
            console.log('addressId2222222222:' + typeof addressId)
            checkedAddress = await this.model('address').where({is_default: 1, user_id: think.userId}).find(); // 默认地址
        } else {
    ...
    /**
     * 立即购买订单提交前的检验和填写相关订单信息
     * @returns {Promise.<void>}
     */
    async onecheckAction() {
        const addressId = this.get('addressId'); // 收货地址id
        // const couponId = this.get('couponId'); // 使用的优惠券id //暂时没使用
        ...
        // 获取要购买的商品
        const cartData = await this.getBuyOne();
        const checkedGoodsList = cartData.cartList.filter(function (v) {
            return v.checked === 1;
        });
